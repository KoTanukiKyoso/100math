    ...
    <div class="mt-4">
      <div v-if="pageType===0" class="pt-4">
        <div v-if="hasEmail" class="text-center pb-4">
          <my-btn prepend-icon="email" min-width="240" text="メール ログイン"
                  color="#DB4437" @click="pageType=1" variant="elevated" dark/>
        </div>
        <div v-if="hasGoogle" class="text-center pb-4">
          <my-btn prepend-icon="google" prepend-icon-color="#ea4335"
                  min-width="240" text="Google ログイン" color="undefined"
                  @click="authLogin('google')" variant="elevated"/>
        </div>
        <div v-if="hasFacebook" class="text-center pb-4">
          <my-btn prepend-icon="facebook" dark
                  min-width="240" text="Facebook ログイン" color="#3b5998"
                  @click="authLogin('facebook')" variant="elevated"/>
        </div>
        <div v-if="hasApple" class="text-center pb-4">
          <my-btn prepend-icon="apple" dark
                  min-width="240" text="Apple Id ログイン" color="#3b5998"
                  @click="authLogin('apple')" variant="elevated"/>
        </div>
      </div>
...
<script setup>
import {
  getAuth, signInWithEmailAndPassword, createUserWithEmailAndPassword, OAuthProvider,
  GoogleAuthProvider, TwitterAuthProvider, FacebookAuthProvider, sendEmailVerification,
  signInWithRedirect, updateProfile, sendPasswordResetEmail, setPersistence, browserLocalPersistence
} from "firebase/auth";
...
const {isSignedIn, user, addMessage, onAuthStateChanged} = useStore();
...
const signup = () => {
  ...
  if (!process.env.DEBUG && email.value.indexOf("+") >= 0) {
    addMessage({
...
const sendVerify = (user = null) => {
  ...
  sendEmailVerification(user).then(() => {
    addMessage({
      text: `確認メールを送信しました，届かない場合は迷惑メールフォルダをご確認ください`,
      risk: 0,
      dismissible: true,
    });
  });
...
const sendPassResetMail = () => {
  ...
  sendPasswordResetEmail(auth, email.value).then(() => {
    addMessage({
      text: `「${email.value}」に再設定メールを送信しました，届かない場合は迷惑メールフォルダをご確認ください`,
      risk: 0,
      dismissible: true,
    });
    email.value = "";
...
//
watch(isSignedIn, async (now, prev) => {
  if (now) {
    router.push(to);
  }
});
